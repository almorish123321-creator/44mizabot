...
"""
╔═══════════════════════════════════════════════════════════════╗
║     🤖 بوت النشر الخارق - النسخة المتطورة لمكافحة الكشف 💪   ║
║     مع تقنيات متقدمة لتجاوز جميع بوتات الحماية              ║
╚═══════════════════════════════════════════════════════════════╝
"""
...
class AntiDetection:
    """كلاس متخصص في تجنب اكتشاف بوتات الحماية:
    @GHSecurityBot, @masah3bot, @SaudiFalconBot, @Auto_AnswersBOT, 
    @Jabal_RoBot, @Saudi_Education_Group_bot, @GHClone4Bot
    """
    
    def __init__(self):
        # سجل الرسائل المنشورة لمنع التكرار
        self.posted_messages = {}
        self.last_posts = {}
        self.group_activity = {}  # تتبع نشاط المجموعات
        
        # مرادفات للكلمات المفتاحية (للتضليل)
        self.synonyms = {
            'اشترك': ['انضم', 'تابع', 'كن معنا', 'سجل', 'ادخل', 'شارك'],
            'قناة': ['مجموعة', 'منصة', 'صفحتنا', 'مكاننا', 'رابطنا', 'المكان'],
            'دعم': ['مساندة', 'متابعة', 'تفاعل', 'تشجيع', 'اهتمام'],
            'حصري': ['مميز', 'خاص', 'فريد', 'استثنائي', 'نادر'],
            'خصم': ['تخفيض', 'عرض', 'فرصة', 'توفير', 'كود'],
            'متابعين': ['أعضاء', 'زوار', 'جمهور', 'مشتركين', 'رواد'],
            'رابط': ['وصلة', 'لينك', 'عنوان', 'مكان', 'دليل'],
            'تليجرام': ['تيليجرام', 'تلجرام', 'التطبيق', 'الماسنجر', 'تلغرام'],
            'بوت': ['روبوت', 'الخدمة', 'الأداة', 'المساعد', 'برنامج'],
            'عرض': ['فرصة', 'تخفيض', 'هدية', 'منحة', 'مبادرة'],
            'مجاني': ['بدون رسوم', 'هدية', 'بالمجان', 'حر', 'مفتوح'],
        }
        
        # كلمات ممنوعة يجب تجنبها أو استبدالها
        self.forbidden_words = [
            'اشتراك', 'اشترك', 'قناة', 'بوت', 'دعم', 'متابعين', 'خصم',
            'حمل', 'تنزيل', 'رابط', 'لينك', 't.me', 'telegram.me',
            'انضم', 'سجل', 'عرض', 'حصري', 'تخفيض', 'فرصة', 'هدية'
        ]
        
        # رموز غير مرئية للتشفير المتقدم
        ...
        # قوالب مختلفة للرسائل لتجنب النمط الثابت
        self.templates = [
            "{}",
            "✨ {} ✨",
            "🔹 {}\n🔸 تابعنا للمزيد",
        ...
        # رسائل تمهيدية للتفاعل
        self.warm_messages = [
            "👍", "❤️", "🔥", "👏", "🌟", "✨",
            "ممتاز 🙏", "رائع 🌟", "جميل 💫", "شكراً 🎉",
            "😊👍", "💪🔥", "🎯✨", "⭐️🌟"
        ]
        
        # أنماط الروابط لاكتشافها
        self.link_patterns = [
            r't\.me/[a-zA-Z0-9_]+',
            r'telegram\.me/[a-zA-Z0-9_]+',
            r'https?://t\.me/[a-zA-Z0-9_]+',
            r'https?://telegram\.me/[a-zA-Z0-9_]+',
            r'@[a-zA-Z0-9_]+',  # المعرفات
        ]
        
        # التشكيل (الحركات) للتضليل
        self.tashkeel = ['َ', 'ُ', 'ِ', 'ً', 'ٌ', 'ٍ', 'ّ', 'ْ']
        
        logger.success("✅ تم تهيئة نظام مكافحة الاكتشاف")
    
    def disguise_text(self, text):
        """تغيير النص بشكل متقدم ليتجنب جميع الفلاتر"""
        result = text
        ...
        # إضافة تشكيل عشوائي للتضليل
        words = result.split()
        for i in range(len(words)):
            if len(words[i]) > 2 and random.random() > 0.7:
                # أضف حركة عشوائية في منتصف الكلمة
                pos = random.randint(1, len(words[i])-1)
                words[i] = words[i][:pos] + random.choice(self.tashkeel) + words[i][pos:]
        result = " ".join(words)
        
        # تغيير ترتيب الكلمات أحياناً
    ...
    def disguise_link(self, text):
        """إخفاء الروابط والمعرفات بشكل متقدم"""
        result = text
        ...
        for pattern in self.link_patterns:
            ...
            for match in matches:
                if random.random() > 0.7:
                    # استبدال الرابط بنص إرشادي
                    result = result.replace(match, "🔍 ابحث عن المعرف في البحث")
                else:
                    # تجزئة الرابط بأحرف غير مرئية
                    disguised = match.replace('.', '.\u200B').replace('/', '/\u200B').replace('@', '@\u200B')
                    result = result.replace(match, disguised)
        
        return result
    
    def random_delay(self, base_delay=3):
        """تأخير عشوائي متقدم لتجنب الأنماط"""
        # تأخير بين 60% و 200% من الوقت الأساسي
        return random.randint(int(base_delay * 0.6), int(base_delay * 2))
    
    def get_variation(self, text, variation_count=15):
        """توليد تنويعات متعددة لنفس النص"""
        variations = set()
        for _ in range(variation_count * 2):  # توليد ضعف العدد ثم اختيار فريد
            variant = text
    ...
    def add_invisible_chars(self, text):
        """إضافة أحرف غير مرئية بشكل عشوائي متقدم"""
        ...
        # أضف أحرف غير مرئية بين الكلمات أحياناً
        if random.random() > 0.8:
            final_result = []
            for i, word in enumerate(result):
                final_result.append(word)
                if i < len(result) - 1 and random.random() > 0.7:
                    final_result.append(random.choice(self.invisible_chars) * random.randint(1, 3))
            return " ".join(final_result)
        
        return " ".join(result)
    
    def is_safe_to_post(self, group_id, text):
        """التحقق من أمان النشر في المجموعة"""
        now = datetime.now()
        
        # التحقق من آخر نشر في نفس المجموعة
        if group_id in self.last_posts:
            last_time = self.last_posts[group_id]
            # تجنب النشر في نفس المجموعة خلال 45 دقيقة
            if (now - last_time).total_seconds() < 2700:
                return False, "تم النشر مؤخراً في هذه المجموعة"
        
        # التحقق من وجود كلمات ممنوعة
        cleaned_text = text.lower()
        for word in self.forbidden_words:
            if word in cleaned_text and random.random() > 0.5:
                # استبدال الكلمة الممنوعة
                text = text.replace(word, '***')
        
        return True, text
    
    async def send_safe(self, client, chat_id, original_text, group_name=""):
        """إرسال آمن مع جميع تقنيات التمويه"""
        try:
            # التحقق من الأمان
            safe, processed = self.is_safe_to_post(chat_id, original_text)
            if not safe:
                return False, processed
            
            # اختيار تنويع عشوائي
            variations = self.get_variation(processed, 8)
            ...
            # إرسال بدون أزرار (الأزرار تسبب الاكتشاف)
            await client.send_message(chat_id, final_text)
            
            # تسجيل وقت النشر
            self.last_posts[chat_id] = datetime.now()
            
            logger.info(f"📨 تم إرسال رسالة موهّمة لـ {group_name[:30]}")
            return True, "تم النشر بنجاح مع تقنيات التمويه"
            
        except FloodWaitError as e:
            logger.warning(f"⏳ انتظار {e.seconds} ثانية...")
    ...
    def get_warm_up_message(self):
        """الحصول على رسالة تمهيدية عشوائية"""
        return random.choice(self.warm_messages)
    
    async def send_warm_up(self, client, chat_id, group_name=""):
        """إرسال تفاعل تمهيدي لتجنب الاشتباه"""
        if random.random() > 0.75:  # 25% فقط من المجموعات
            try:
                await client.send_message(chat_id, self.get_warm_up_message())
                logger.info(f"🔥 تم إرسال تفاعل تمهيدي لـ {group_name[:30]}")
                await asyncio.sleep(random.randint(5, 20))
                return True
            except Exception as e:
                logger.warning(f"⚠️ فشل إرسال تفاعل تمهيدي: {e}")
        return False
    
    def should_send_warm_up(self, group_id):
        """تحديد ما إذا كان يجب إرسال تفاعل تمهيدي"""
        # لا ترسل تفاعل تمهيدي إذا كان هناك تفاعل سابق
        if group_id in self.group_activity:
            last_activity = self.group_activity[group_id]
            if (datetime.now() - last_activity).total_seconds() < 3600:
                return False
        return random.random() < 0.2  # 20% فرصة
    
    def record_activity(self, group_id):
        """تسجيل نشاط في المجموعة"""
        self.group_activity[group_id] = datetime.now()
    
    def get_rotating_account(self, accounts_list):
        """تدوير الحسابات لتجنب الأنماط"""
        if not accounts_list:
            return None
        return random.choice(accounts_list)
    
    def split_long_message(self, text, max_length=400):
        """تقسيم الرسائل الطويلة لتجنب الاشتباه"""
    ...
    def add_random_emojis(self, text):
        """إضافة إيموجي عشوائي للتضليل"""
        emojis = ['✨', '🌟', '⭐️', '💫', '🔥', '⚡️', '🎯', '💎', '🔔', '📢']
        if random.random() > 0.6:
            return f"{random.choice(emojis)} {text} {random.choice(emojis)}"
        return text

# ==================== إنشاء كائن مكافحة الاكتشاف ====================
...
async def start_handler(event):
    ...
    await event.respond(
        ...
        f"🔐 **التشفير:** {'مفعل' if SETTINGS.get('encryption', True) else 'معطل'}\n\n"
...
async def callback_handler(event):
    ...
    if data == "status":
        ...
        await event.edit("⏱ أرسل الفاصل الزمني (5-120 ثانية):"); 
        TEMP[ADMIN_ID] = "time"
    ...
    # إدارة الرسائل
        ...
        await event.edit("📝 **إدارة الرسائل المتعددة**", buttons=messages_buttons())
    ...
    # التقارير
        ...
        await event.edit("📊 **التقارير الحقيقية**", buttons=reports_buttons())
    ...
    # التحكم في النشر
        ...
        await event.edit("🚀 بدأ النشر بنظام مكافحة الكشف", buttons=main_buttons())
    elif data == "stop_p":
        is_posting = False
    ...
    # معالجة ضبط الوقت
        ...
        try:
            ...
            if 5 <= interval <= 120:
                ...
                await event.respond("❌ الرجاء إدخال قيمة بين 5 و 120")
        except:
...
# دالة الانضمام التلقائي مع حفظ البيانات
    ...
    for link in links:
        for phone, client in USER_CLIENTS.items():
            try:
                ...
                # تسخين المجموعة بعد الانضمام
                if SETTINGS.get('warm_up_enabled', True):
                    await asyncio.sleep(random.randint(5, 15))
                    try:
                        await client.send_message(group_id, anti_detection.get_warm_up_message())
                    except:
                        pass
...
# ===== دالة النشر المتطورة مع مكافحة الكشف =====
async def poster():
    global is_posting
    ...
    while is_posting:
        try:
            ...
            # توليد تنويعات للرسالة
            if SETTINGS.get('anti_detection', True):
                variations = anti_detection.get_variation(original_text, 12)
                logger.info(f"📝 تم توليد {len(variations)} تنويع مختلف")
            else:
            ...
            for phone, client in accounts_list:
                ...
                try:
                    ...
                    async for dialog in client.iter_dialogs():
                        if dialog.is_group and groups_sent < SETTINGS.get('max_groups_per_account', 50):
                    ...
                    for dialog in groups_list:
                        ...
                        # إرسال تفاعل تمهيدي (إذا كانت الميزة مفعلة)
                        if SETTINGS.get('warm_up_enabled', True) and SETTINGS.get('anti_detection', True):
                            if anti_detection.should_send_warm_up(dialog.id):
                                await anti_detection.send_warm_up(client, dialog.id, dialog.name)
                        
                        # اختيار رسالة عشوائية من التنويعات
                        ...
                        # تأخير عشوائي
                        delay = anti_detection.random_delay(SETTINGS.get('interval', 5)) if SETTINGS.get('anti_detection', True) else SETTINGS.get('interval', 5)
            ...
            # استراحة عشوائية بين الدورات
            cycle_wait = random.randint(60, 180)
...
async def main():
    ...
    print("🎉 البوت يعمل مع نظام مكافحة الكشف المتطور")
